...
int dct_compute()
{
	...
	*(bram_arm_control + 0) = 0x1; //Write Enable High
	...
	for(int idx = 0; idx <64; idx ++)
	{
		...
		*(bram_arm_control + 2) = sample_block[4][idx]; // Data
...
int quantz_compute()
{
	...
	*(bram_arm_control + 0) = 0x1; //Write Enable High
	*(bram_arm_control + 1) = 0x3F; // Address
...
int rle_compute()
{
	...
	*(arm_fpga_control_bus + 7) = 0x1; // Start Computation
	...
	bitlength[0] = *(arm_fpga_control_bus + 8);
	printf("BitLength: %d\n", bitlength[0]);
	...
	*(bram_arm_control + 0) = 0x1; //Write Enable High
	*(bram_arm_control + 1) = 0; // Address
	*(bram_arm_control + 2) = 0x4; // Data
...
int huffman_compute()
{
	...
	*(arm_fpga_control_bus + 7) = 0x1; // Start Computation
	...
	hufflength[0] = *(arm_fpga_control_bus + 8);
	printf("HuffLength: %d\n", hufflength[0]);
...
int main()
{
	...
	*(arm_fpga_control_bus + 9) = bitlength[0]; // RLE Length
	set_switch_module();
	huffman_reconfig();
	huffman_compute();
