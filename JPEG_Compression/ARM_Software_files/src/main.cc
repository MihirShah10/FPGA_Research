...
bool huffman_encoded_out [64*64][512];
...
int trigger_huffman(int column_index, int row_index)
{
	...
	*(HUFFMAN + 129) = rle_max_length_buffer[column_index + (row_index*64)];

	for(int idx = 0; idx <128; idx ++)
	{
		*(HUFFMAN + idx + 1) = huffman_data_buffer_input[128*(column_index + (row_index*64)) + idx] ;//*(value + idx);
	}

	while(!(*(HUFFMAN + 511)==1));// Check Valid Signal is active or not

	for(int i = 10000; i >0; i--){;} // 10k original, 1000 gives 13 us delay
	huffman_last_bit[column_index + (row_index*64)] = *(HUFFMAN + 510);
	for(int i = 1000; i >0; i--){;} // 10k original, 1000 gives 13 us delay

	if(huffman_last_bit[column_index + (row_index*64)] > 380)
	{
		printf("Last Bit[%d]: %d\n",(column_index + (row_index*64)),huffman_last_bit[column_index + (row_index*64)]);
	}
	//printf("Last Bit[%d]: %d\n",(column_index + (row_index*64)),huffman_last_bit[column_index + (row_index*64)]);
	for(int idx = 0; idx < (huffman_last_bit[column_index + (row_index*64)]); idx ++)
	{
		huffman_encoded_out[column_index + (row_index*64)][idx] = (bool *)*(HUFFMAN + 130 + idx);
		//printf("huffman_encoded_out[%d]: %d\n",idx, (char)(huffman_encoded_out[column_index + (row_index*64)][idx]));
	}
...
int main()
{
	...
	timer.stopTimer();
	timeSW = timer.getElapsedTimerInSeconds();
	printf("Huffman Completed in %f seconds\n", timeSW);

	FILE *fp;
	fp=fopen("test.txt", "wb");
	char h[10]= "ABCDEF";
	fwrite(h, sizeof(h[0]), sizeof(h)/sizeof(h[0]), fp);
	fclose(fp);
